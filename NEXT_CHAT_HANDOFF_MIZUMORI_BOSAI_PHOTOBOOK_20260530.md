...
## 2026-05-30 高解像度写真差し替え

### 対応理由
- 初回版はChatGPT生成の24分割コンタクトシートから切り出していたため、写真を大きく表示すると細部がぼやけた。
- 画面レイアウト上も選択写真が右側パネル内で小さく見えていた。

### 対応内容
- 写真24枚をChatGPTで個別生成し直し、既存の `photo-01-...jpg` から `photo-24-...jpg` を差し替え。
- 各写真は `1586x992` の個別画像。
- プロジェクト内には個別写真のみを配置。スプライトシートは未使用。
- PC表示では写真一覧をコンパクト化し、選択写真側を大きく表示するレイアウトへ変更。
- キャッシュ更新用クエリを `20260530-highres` に変更。
...
### 対応理由
- ユーザーより「確認済み」は不要、あたかも本当に存在するサイトのように、メタ要素なし、色は現代風にしたいとの指摘。

### 対応内容
- 写真カードの「確認済」バッジを削除。
- 「手がかり」「最終推理」「提出」「正解」「調査印」など、謎解き用UIに見える語を表向きの自治体サイト風に変更。
- 表記を「注記」「資料照会」「情報公開請求用メモ」「内容を整理」などへ変更。
- 配色をベージュ系から、白、淡いブルーグレー、濃紺、ティール中心の現代的な公共情報サイト風へ変更。
- `app.js` 内の動的日本語は文字化け防止のためUnicodeエスケープ形式へ統一。
- キャッシュ更新用クエリを `20260530-modern-public` に変更。

### 確認
- `node --check MizumoriBosaiPhotobookMystery/web/app.js` 成功。
- `確認済`、`謎`、`手がかり`、`調査`、`最終推理`、`正解`、`ゲーム`、`プレイ`、`ホットスポット`、`クリック` などのメタ寄り語句は `web` 配下で検索結果なし。
- ローカルHTTPで `index.html`、`style.css`、`app.js` がHTTP 200。
