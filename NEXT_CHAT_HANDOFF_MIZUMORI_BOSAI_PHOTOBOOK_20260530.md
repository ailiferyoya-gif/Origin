...
## 2026-05-30 写真注記と拡大表示の調整
### 対応理由
- ユーザーより、画像と参照内容が一致していない、画像をもっと拡大できるようにしたいとの指摘。
- 画像に写っていない細かな文字や番号を読ませる設計を避け、写真内に実際に見える物体・色・配置を拾う注記へ寄せた。

### 対応内容
- 写真24枚の注記を、赤い帯、右向き矢印、赤いコーン、泥跡、棚、中央部、黄色い旗、長靴、白い札、赤と緑のランプ、黄色い札、暗い窓、濡れた線、金属扉、赤い灯りなど、画像上で確認しやすい要素へ変更。
- 各章の資料照会も、写真上の要素を入力する形式へ変更。
- 写真詳細に「拡大」ボタンを追加。
- 写真本体クリックでも拡大ビューを開けるようにした。
- 全画面の拡大ビューを追加し、縮小、等倍、拡大、閉じるを操作可能にした。
- 拡大時は画像の表示幅そのものを変更し、拡大後に上下左右へスクロールして確認できるようにした。
- キャッシュ更新用クエリを `20260530-zoom-rework` に変更。
