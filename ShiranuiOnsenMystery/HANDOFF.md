...
## 2026-05-24 公式サイト 下層ページ内容拡充・トップ案内一覧修正

- ユーザー指摘: 下層ページの内容量が少ない。トップページの「湯/泊/道/守/帳/告」案内一覧のレイアウトが不自然。
- バックアップ: `C:\Users\kogit\Documents\Codex\backups\shiranui-onsen-content-expanded-20260524`
- 変更内容:
  - トップページの公式ページ一覧を、単独一文字ラベルではなく「湯の案内」「泊まりの案内」「道しるべ」などの読みやすいラベルへ変更。
  - 公式ページ一覧のCSSを縦文字風から通常の3列案内リストへ変更。
  - `温泉`、`客室と食事`、`館内・アクセス`、`館内安全`、`鍵と宿帳`、`お知らせ` の各下層ページに、案内表・時刻控え・関連ページ導線・不穏な記録要素を追加。
  - トップ見出しの「夜がほどける。」が不自然に割れないよう `.nobr` を追加。
  - 残っていた英字ラベルを日本語ラベルへ統一。
- 確認:
  - 公式7ページで画像切れなし。
  - 公式7ページで横スクロールなし。
  - 公式7ページで英字大文字ラベルパターンなし。
  - トップ見出しの不自然な一文字改行なし。
  - 公式ページ内リンク49件がすべてHTTP 200。
- 未完了事項:
  - 実機スマホの目視確認は未実施。
- 次にやること:
  - 必要に応じて、各下層ページに謎解き用の調査導線や隠し記録への入り口を追加する。

## 2026-05-24 公式サイト 滞在中の流れカード余白修正

- ユーザー指摘: `客室と食事` ページの「15:00 帳場でチェックイン」などの文字が枠ぎりぎりにある。
- バックアップ: `C:\Users\kogit\Documents\Codex\backups\shiranui-route-padding-fix-20260524`
- 変更内容:
  - `.sh-route article` に左右24pxの余白を追加。
  - ルートカードに薄い背景を付け、枠線だけの窮屈な見え方を緩和。
  - スマホ幅では `.sh-route article` を18px余白に調整。
- 確認:
  - `客室と食事` ページの該当カードで左右余白24pxを確認。
  - 画像切れなし。
  - 横スクロールなし。

## 2026-05-24 公式サイト完成ロック

- ユーザー確認: 公式サイトはOK。完成扱いとしてロック。
- 除外対応: ルートの `COMPLETED_SITE_EXCLUSIONS_20260524.md` に `ShiranuiOnsenMystery` を追加。
- 今後の扱い:
  - ユーザーが `ShiranuiOnsenMystery` を明示的に指定しない限り、「すべて」「全部」「全件」の公式サイト更新対象に含めない。
  - 次工程は本格的な謎解き作成確認へ移る。
